...
export function SceneAiPreviewPanel({
  ...
  const isPending = runAgent.isPending || runBatch.isPending;
  const previewQuery = useScenePreview(projectId, episodeId, { pollWhileRunning: isPending });
  ...
  const runSingleScene = (sceneId: string) => {
    runAgent.mutate({
      agentId,
      sceneId,
      episodeId,
  ...
  const handleRunSelected = () => {
    ...
    if (selectedSceneIds.length === 1) {
      runSingleScene(selectedSceneIds[0]);
      return;
    }
  ...
  return (
    ...
    <Card>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <CardTitle className="text-base">
              {agentId === 'video' ? 'Generated videos & scene preview' : 'Scene preview for AI'}
            </CardTitle>
            <p className="text-muted-foreground mt-1 text-sm">
              {agentId === 'video'
                ? 'Each regenerate adds a new version. Pick the best take — you do not need to wait for other scenes.'
      ...
      <CardContent className="space-y-4">
        {agentId === 'video' && totalVideoCount > 0 && (
        ...
        {isPending && (
          <p className="text-muted-foreground text-sm">
            Generation in progress… new versions appear below as each scene completes.
          </p>
        )}
                        ...
                        {agentId === 'video' && (
                          <div className="flex flex-wrap gap-2">
                            <Button
                              ...
                              size="sm"
                              disabled={isPending}
                              onClick={() => runSingleScene(scene.id)}
                            >
                              {hasVideo ? (
                                <>
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            type="button"
            disabled={isPending || selectedSceneIds.length === 0}
          ...
          {episodeId && (
            ...
            >
              ...
              {scenesWithVideo.length === scenes.length && agentId === 'video'
        ...
        {(runAgent.isError || runBatch.isError) && (
          <p className="text-destructive text-sm">
            {(runAgent.error as Error)?.message ??
              (runBatch.error as Error)?.message ??
              'Generation failed.'}
          </p>
        )}
        {selectVideo.isError && (
